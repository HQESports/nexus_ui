...
import {
  FilteredMatchesParams,
  FilteredMatchesResponse,
        ...
        <Sidebar side="right" className="border-l">
          ...
          <SidebarContent>
            <MatchFilter
              ...
              count={matchesResponse.count}
            />
            <SidebarGroup className="space-y-2 border-b-2">
              {/* Phase selectors */}
              <div className="text-lg w-full text-center text-primary font-bold">
                Phase Controls
              </div>
                      ...
                      >
                        <Checkbox
                          ...
                          onCheckedChange={(checked) =>
                            handlePhaseChange(option.id, checked as boolean)
                          }
                        />
                        <Label
                          htmlFor={`phase-${option.id}`}
                          className="cursor-pointer text-sm font-normal flex-1"
                        >
                          {option.label}{" "}
                          <Badge className="bg-blue-500 text-white border-blue-700">
                            {phaseCount.get(option.id)}
                          </Badge>
                        </Label>
                      </div>
                    ))}
            ...
            <SidebarGroup className="space-y-2">
              ...
              {mode === AnalyzeModes.Cluster && (
                <div className="space-y-2">
                  <Label>
                    Cluster Radius{" "}
                    {convertUnitsToMeters(clusterRadius / PUBG_MAP_SCALE)}m
                  </Label>
                  <Slider
                    value={[clusterRadius]}
